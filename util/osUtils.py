...
class OsUtils:
    ...
    def check_platform(self):
        ...
        if self.helper.string_equals_ignore_case(system, 'Linux'):
            if self.helper.string_contains_ignore_case('Microsoft', platform.release()):
                self.helper.print_text('Keep in mind that', self.appOptions.appName, 'was not tested under WSL.')
            else:
                self.helper.print_text(self.appOptions.appName, 'is running under *Unix... good.')
        elif self.helper.string_equals_ignore_case(system, 'Windows'):
            self.helper.print_text(self.appOptions.appName, 'is running under Windows. This is not a really smart idea.')
            sys.exit(0)
        else:
    ...
    def check_privileges(self):
        if os.geteuid() != 0:
            self.helper.print_text('Information:', self.appOptions.appName, 'is not running as root. Good.')
